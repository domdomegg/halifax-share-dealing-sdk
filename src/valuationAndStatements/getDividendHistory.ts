import type {
	Accounty, Config, Dividend, DividendOption,
} from '../types';
...
export default (config: Config) => async ({accountId}: Accounty): Promise<Dividend[]> => request(`${urlBuilder(config).generateSD('sdaccountdividendstatement', accountId)}&ChosenStartMonth=1&ChosenStartYear=2005&ChosenEndMonth=12&ChosenEndYear=3000`)
